...
## Set starting Q-values.
qvalues = [{0: 1,
          1: 0},
          {0: 0,
           1: 0}]
for i in range(len(agents)):
    for key in agents[i]._q_values.keys():
        for key2 in collections.defaultdict(float).keys():
            agents[i]._q_values[key][key2] = qvalues[i]
print("values:", [agent._q_values.values() for agent in agents])
