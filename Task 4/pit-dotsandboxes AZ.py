...
flags.DEFINE_integer("max_simulations", 150, "How many simulations to run.")
...
def main(unused_argv):
    ...
    model.from_checkpoint("checkpointAZ/checkpoint-60")
    evaluator = evaluator_lib.AlphaZeroEvaluator(game, model)
    rp2 = MCTSBot(game,FLAGS.uct_c,FLAGS.max_simulations,evaluator)
    print("loaded AZ")

    # loading the trained model
    n1 = gnn()
    n1.load_checkpoint("checkpoint", "best.h5")
    n1p = MCTSBot(game,args.cpuct,args.numMCTSSims,GNNEvaluator(n1,args))
    print("loaded gnn")

    p1 = n1p
    p2 = rp2
    arena = Arena.Arena(p1,p2, game)
    oneWon, twoWon, draws = arena.playGames(games_to_play)

    print("oneWon: {}, twoWon: {}, draws: {}".format(oneWon, twoWon, draws))

    arena = Arena.Arena(p2,p1, game)
    arena.playGamesAgainstRandom(p2,games_to_play)
